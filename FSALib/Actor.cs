...
namespace FSALib
{
    ...
    public struct Actor : IComparable<Actor>
    {
        ...
        public static bool PasteFromString(string actor, out Actor newActor)
        {
            ...
            newActor.Variable1 = byte.Parse(match.Groups[5].Value);
            newActor.Variable2 = byte.Parse(match.Groups[6].Value);
            newActor.Variable3 = byte.Parse(match.Groups[7].Value);
            newActor.Variable4 = byte.Parse(match.Groups[8].Value);
            return true;
        }

        /// <inheritdoc/>
        public readonly string ToStringCode()
            => $"{ID}, L:{Layer}, X:{XCoord}, Y:{YCoord}, V:{Variable1},{Variable2},{Variable3},{Variable4}.";
